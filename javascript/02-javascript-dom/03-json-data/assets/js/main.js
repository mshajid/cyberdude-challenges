  ...
  <div class="flex items-center justify-center gap-x-5 py-4">
    ...
    <div class="space-y-2">
      <div class="leading-3">
        ...
        <span id="location" class="text-sm tracking-tight">${
          person.location
        }</span>
      </div>
      <div class="leading-3">
        <h2 id="position" class="font-semibold text-sm">${
          person.role.position
        }</h2>
        <span class="font-bold">@</span>
        <span id="organization" class="font-light text-xs -tracking-tighter">${
          person.organization
        }</span>
      </div>
    </div>
...
>
  <div class="flex gap-x-2 border-r-2 px-5 py-5">
    <div class="flex flex-col leading-tight">
      <span id="totalJobs" class="font-bold text-xl">${
        person.role.totalJobs
      }</span>
      <span class="font-light text-sm">Total Jobs</span>
    </div>
    <div class="flex flex-col leading-tight">
      <span id="totalHours" class="font-bold text-xl">${
        person.role.totalHours
      }</span>
  ...
  <div class="px-10 flex flex-col gap-y-3">
    ...
    <div>
      ...
      <button title="Hire ${
        person.name
      }" class="bg-[#A78295] px-2 py-0.5 my-2 rounded-md text-white text-sm">Hire Now</button>
    </div>
  </div>
`;
